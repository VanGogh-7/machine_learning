...
from src.model import DCGANDiscriminator, DCGANGenerator, weights_init_dcgan
...
def main() -> None:
    ...
    discriminator = DCGANDiscriminator(
        image_channels=config.image_channels,
        feature_maps=config.discriminator_feature_maps,
    ).to(device)
    generator.apply(weights_init_dcgan)
    discriminator.apply(weights_init_dcgan)

    real_images, _ = next(iter(train_loader))
    noise = torch.randn(real_images.size(0), config.latent_dim, 1, 1, device=device)
    generator.eval()
    discriminator.eval()
    with torch.no_grad():
