...
def plot_training_curves(
    ...
    if any(value is not None for value in history.get("train_aucs", [])):
        axes[2].plot(epochs, history["train_aucs"], label="Training")
    if any(value is not None for value in history.get("valid_aucs", [])):
        axes[2].plot(epochs, history["valid_aucs"], label="Validation")
