...
def train_one_epoch(
    ...
    for target_items, histories, masks, targets in tqdm(data_loader, leave=False):
        ...
        optimizer.zero_grad()
        logits = model(target_items, histories, masks)
...
def evaluate(
    ...
    with torch.no_grad():
        for target_items, histories, masks, targets in data_loader:
            target_items = target_items.to(device)
            histories = histories.to(device)
            masks = masks.to(device)
            targets = targets.to(device)
