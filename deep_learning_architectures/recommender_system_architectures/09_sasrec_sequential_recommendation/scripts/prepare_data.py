...
PADDING_ITEM_ID = 0
CSV_COLUMNS = ["input_item_ids", "target_item_id", "label"]
...
def encode_items(
    ...
    return encoded, raw_movie_id_to_item_id, item_id_to_title


def format_history(history: list[int], max_sequence_length: int) -> str:
    truncated = history[-max_sequence_length:]
    padding_length = max_sequence_length - len(truncated)
    input_item_ids = [PADDING_ITEM_ID] * padding_length + truncated
    return " ".join(str(item_id) for item_id in input_item_ids)
...
def sample_negative_item(
    ...
    for _ in range(max_trials):
        candidate = int(rng.integers(1, num_items))
        if candidate != PADDING_ITEM_ID and candidate not in user_interacted_items:
            return candidate
    return None
...
def build_samples(
    ...
    num_items = int(positive_ratings["item_id"].max()) + 1
    samples: list[tuple[str, int, int]] = []

    sorted_ratings = positive_ratings.sort_values(["userId", "timestamp"])
    for _, user_history in sorted_ratings.groupby("userId", sort=False):
        item_sequence = user_history["item_id"].astype(int).tolist()
        if len(item_sequence) < config.min_sequence_length:
            continue

        user_interacted_items = set(item_sequence)
        for target_position in range(1, len(item_sequence)):
            history = item_sequence[:target_position]
            target_item_id = item_sequence[target_position]
            input_item_ids = format_history(history, config.max_sequence_length)
            samples.append((input_item_ids, target_item_id, 1))
            ...
            for _ in range(config.negative_samples_per_positive):
                ...
                if negative_item_id is not None:
                    samples.append((input_item_ids, negative_item_id, 0))
                    if (
                        config.max_samples is not None
                        and len(samples) >= config.max_samples
                    ):
                        return samples[: config.max_samples]
...
def split_samples(
    ...
    if valid_size + test_size >= len(shuffled):
        raise ValueError("The generated SASRec dataset is too small for the split.")
...
def write_split(path: Path, rows: list[tuple[str, int, int]]) -> None:
...
def build_metadata(
    ...
    metadata: dict[str, Any] = {
        ...
        "num_items": len(raw_movie_id_to_item_id) + 1,
        ...
        "task_type": "sequential_next_item_binary_prediction",
    }
...
def main() -> None:
    ...
    positive_ratings, raw_movie_id_to_item_id, item_id_to_title = encode_items(
        positive_ratings,
        movies,
    )
    samples = build_samples(positive_ratings, config)
    if not samples:
        raise ValueError(
            "No SASRec samples were generated. Try lowering positive_rating_threshold "
            "or min_sequence_length."
        )
