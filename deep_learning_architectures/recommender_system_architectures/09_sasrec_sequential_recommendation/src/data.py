...
REQUIRED_PROCESSED_COLUMNS = {"input_item_ids", "target_item_id", "label"}


class ProcessedSASRecDataset(Dataset):
    def __init__(self, csv_path: Path, max_sequence_length: int) -> None:
    ...
    def __getitem__(
        ...
        input_item_ids = parse_sequence(
            value=row["input_item_ids"],
...
def parse_sequence(value: Any, expected_length: int) -> torch.Tensor:
    values = str(value).split()
    if len(values) != expected_length:
        raise ValueError(
            f"input_item_ids must contain {expected_length} values, "
            f"but found {len(values)}."
        )
    return torch.tensor([int(item) for item in values], dtype=torch.long)
...
def require_processed_files(config: TrainConfig) -> None:
    ...
    raise FileNotFoundError(
        "Processed SASRec CSV files were not found.\n"
        f"Missing files:\n{missing_text}\n"
        "Please run:\n"
...
def build_dataloaders(
    ...
    train_dataset = ProcessedSASRecDataset(
        config.processed_train_path,
        max_sequence_length=max_sequence_length,
    )
    valid_dataset = ProcessedSASRecDataset(
        config.processed_valid_path,
        max_sequence_length=max_sequence_length,
    )
    test_dataset = ProcessedSASRecDataset(
        config.processed_test_path,
        max_sequence_length=max_sequence_length,
    )
