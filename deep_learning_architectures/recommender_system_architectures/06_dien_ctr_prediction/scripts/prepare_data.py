...
def validate_config(config: TrainConfig) -> None:
    ...
    if config.min_history_length < 2:
        raise ValueError("min_history_length must be at least 2 for DIEN aux loss.")
    if (
...
def encode_items(
    ...
    encoded["item_id"] = encoded["movieId"].map(raw_movie_id_to_item_id).astype("int32")

    item_id_to_title: dict[int, str] = {}
    for row in movies[movies["movieId"].isin(set(raw_movie_id_to_item_id))].itertuples(
        index=False
    ):
...
def format_sequences(
    ...
    current_positive_item_id: int,
    max_history_length: int,
) -> tuple[str, str, str, str]:
    truncated_history = positive_history[-max_history_length:]
    shifted_next_items = positive_history[1:] + [current_positive_item_id]
    truncated_next_items = shifted_next_items[-max_history_length:]

    padding_length = max_history_length - len(truncated_history)
    history_item_ids = [PADDING_ITEM_ID] * padding_length + truncated_history
    next_history_item_ids = [PADDING_ITEM_ID] * padding_length + truncated_next_items
    history_mask = [0] * padding_length + [1] * len(truncated_history)
    aux_mask = [0] * padding_length + [1] * len(truncated_next_items)

    return (
        " ".join(str(item_id) for item_id in history_item_ids),
        " ".join(str(item_id) for item_id in next_history_item_ids),
        " ".join(str(mask_value) for mask_value in history_mask),
        " ".join(str(mask_value) for mask_value in aux_mask),
    )
...
def build_samples(
    ...
    samples: list[tuple[int, str, str, str, str, int]] = []
        ...
        for row in user_history.itertuples(index=False):
            ...
            if len(positive_history) >= config.min_history_length:
                (
                    ...
                    positive_history,
                    current_positive_item_id=item_id,
...
def split_samples(
    ...
    if valid_size + test_size >= len(shuffled):
        raise ValueError("The generated DIEN dataset is too small for the split.")
...
def write_split(path: Path, rows: list[tuple[int, str, str, str, str, int]]) -> None:
...
def build_metadata(
    ...
    if config.save_large_metadata:
        metadata["item_id_to_title"] = {
            str(item_id): title
            for item_id, title in sorted(item_id_to_title.items())
        }
        metadata["raw_movie_id_to_item_id"] = {
            str(raw_movie_id): item_id
            for raw_movie_id, item_id in sorted(raw_movie_id_to_item_id.items())
        }
...
def main() -> None:
    ...
    ratings, raw_movie_id_to_item_id, item_id_to_title = encode_items(ratings, movies)
    samples = build_samples(ratings, config)
    if not samples:
        raise ValueError(
            "No DIEN samples were generated. Try lowering positive_rating_threshold "
