...
class ProcessedDIENDataset(Dataset):
    ...
    def __getitem__(
        ...
        next_history_item_ids = parse_sequence(
            row["next_history_item_ids"],
            self.max_history_length,
            dtype=torch.long,
            column_name="next_history_item_ids",
        )
        ...
        aux_mask = parse_sequence(
            row["aux_mask"],
            self.max_history_length,
            dtype=torch.float32,
            column_name="aux_mask",
        )
        return (
...
def require_processed_files(config: TrainConfig) -> None:
    ...
    raise FileNotFoundError(
        "Processed DIEN CSV files were not found.\n"
        f"Missing files:\n{missing_text}\n"
        "Please run:\n"
...
def build_dataloaders(
    ...
    train_dataset = ProcessedDIENDataset(
        config.processed_train_path,
        max_history_length=max_history_length,
    )
    valid_dataset = ProcessedDIENDataset(
        config.processed_valid_path,
        max_history_length=max_history_length,
    )
