...
def encode_positive_pairs(
    ...
    movies: pd.DataFrame,
    config: TrainConfig,
) -> tuple[list[tuple[int, int]], dict[int, str], int, int]:
    ...
    for row in positive_ratings.itertuples(index=False):
        ...
        if config.max_samples is not None and len(pairs) >= config.max_samples:
            break

    return pairs, item_id_to_title, len(raw_user_ids), len(raw_movie_ids)
...
def split_pairs(
    ...
    if len(pairs) < 3:
        raise ValueError("At least three positive pairs are required for splitting.")

    rng = np.random.default_rng(config.seed)
    indices = rng.permutation(len(pairs))
    shuffled = [pairs[int(index)] for index in indices]
    ...
    if valid_size + test_size >= len(shuffled):
        raise ValueError("The generated Two-Tower dataset is too small for the split.")
...
def build_metadata(
    ...
    metadata: dict[str, Any] = {
        ...
        "task_type": "two_tower_retrieval_with_in_batch_negatives",
    }
...
def main() -> None:
    ...
    pairs, item_id_to_title, num_users, num_items = encode_positive_pairs(
        positive_ratings=positive_ratings,
        movies=movies,
        config=config,
    )
    train_rows, valid_rows, test_rows = split_pairs(pairs, config)
    ...
    print(f"Generated positive pairs: {len(pairs)}")
