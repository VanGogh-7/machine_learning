...
from src.model import DCNCTRModel
...
def main() -> None:
    ...
    print(f"Category sizes per field: {metadata['category_sizes']}")
    dcn_input_dim = (
        len(metadata["numerical_feature_names"])
        + len(metadata["category_sizes"]) * config.embedding_dim
    )
    print(f"DCN input dimension: {dcn_input_dim}")
    print(f"Number of cross layers: {config.cross_num_layers}")
