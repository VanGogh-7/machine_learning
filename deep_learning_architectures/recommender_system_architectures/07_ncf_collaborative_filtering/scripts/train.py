...
def main() -> None:
    ...
    if not processed_dataset_exists(config):
        print("Processed NCF dataset not found.")
        print("Run:")
    ...
    print(f"GMF embedding dimension: {config.user_embedding_dim}")
    print(f"MLP embedding dimension: {config.mlp_embedding_dim}")
    print(f"MLP hidden dimensions: {config.mlp_hidden_dims}")
    ...
    model.eval()
    with torch.no_grad():
        logits = model(user_ids.to(device), item_ids.to(device))
