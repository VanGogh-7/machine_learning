...
REQUIRED_PROCESSED_COLUMNS = {"user_id", "item_id", "label"}


class ProcessedNCFDataset(Dataset):
...
def require_processed_files(config: TrainConfig) -> None:
    ...
    raise FileNotFoundError(
        "Processed NCF CSV files were not found.\n"
        f"Missing files:\n{missing_text}\n"
        "Please run:\n"
...
def build_dataloaders(
    ...
    train_dataset = ProcessedNCFDataset(config.processed_train_path)
    valid_dataset = ProcessedNCFDataset(config.processed_valid_path)
    test_dataset = ProcessedNCFDataset(config.processed_test_path)
